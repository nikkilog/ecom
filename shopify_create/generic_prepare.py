...
def _contained_secret_path(secret_home: Path, candidate: Path) -> Path:
    root = secret_home.resolve()
    resolved = candidate.resolve()
    try:
        resolved.relative_to(root)
    except ValueError as exc:
        raise RuntimeError(
            f"Secret path escapes SECRET_HOME: {candidate}"
        ) from exc
    return resolved
...
def _local_secret_home(secret_home: Optional[str]) -> Path:
    configured = _safe_str(secret_home) or _safe_str(
        os.environ.get("SECRET_HOME")
    )
    configured = configured or _safe_str(
        os.environ.get("SHOPIFY_SECRET_HOME")
    )
    path = Path(configured).expanduser() if configured else DEFAULT_LOCAL_SECRET_HOME
    if not path.exists():
        raise RuntimeError(f"SECRET_HOME does not exist: {path}")
    return path.resolve()
...
def _merge_secret_aliases(
    ...
    merged = {
        key: dict(value)
        for key, value in DEFAULT_LOCAL_SECRET_ALIASES.items()
    }
    for key, value in (local_secret_aliases or {}).items():
        merged[str(key)] = {str(k): str(v) for k, v in value.items()}
    return merged
...
def _read_local_secret(
    ...
    *,
    ...
    root = _local_secret_home(secret_home)
    direct_candidates = [
        root / secret_name,
        root / f"{secret_name}.txt",
        root / f"{secret_name}.json",
    ]
    direct_matches = [
        _contained_secret_path(root, candidate)
        for candidate in direct_candidates
        if candidate.is_file()
    ]
    if len(direct_matches) > 1:
        raise RuntimeError(
            f"Multiple direct Secret files found for {secret_name}: "
            f"{direct_matches}"
        )
    if direct_matches:
    ...
    alias = _merge_secret_aliases(local_secret_aliases).get(secret_name)
    if alias:
        mode = _safe_str(alias.get("mode")).upper()
        pattern = _safe_str(alias.get("relative_pattern"))
        if not pattern:
            raise RuntimeError(
                f"Local Secret alias {secret_name!r} has no relative_pattern."
            )
        matches = [
            _contained_secret_path(root, path)
            for path in root.glob(pattern)
            if path.is_file()
        ]
        if len(matches) != 1:
            raise RuntimeError(
                f"Local Secret alias {secret_name!r} expected exactly one "
                f"file for {pattern!r}; found={matches}"
            )
        path = matches[0]
        if mode == "WHOLE_FILE":
            return SecretValue(
        ...
        if mode == "ENV_KEY":
            key = _safe_str(alias.get("key")) or secret_name
            values = _parse_dotenv_values(path).get(key, [])
            if len(values) != 1:
                raise RuntimeError(
                    f"Secret key {key!r} in {path} expected exactly one value."
                )
            return SecretValue(values[0], "LOCAL_ALIAS_ENV", f"{path}:{key}")
        raise RuntimeError(
            f"Unsupported alias mode for {secret_name!r}: {mode!r}"
        )

    exact_values: List[Tuple[Path, str]] = []
    for path in root.rglob("*"):
        if not path.is_file() or path.suffix.lower() not in {
            ...
            continue
        try:
            parsed = _parse_dotenv_values(path)
        except UnicodeDecodeError:
            continue
        for value in parsed.get(secret_name, []):
            exact_values.append((path, value))
    if len(exact_values) == 1:
        path, value = exact_values[0]
        return SecretValue(value, "LOCAL_ENV_FILE", f"{path}:{secret_name}")
    if len(exact_values) > 1:
        raise RuntimeError(
            f"Multiple local values found for Secret {secret_name!r}."
        )
    raise RuntimeError(
        f"Cannot resolve Local Secret {secret_name!r}. Checked direct files, "
        "configured aliases, and exact keys under SECRET_HOME."
    )
